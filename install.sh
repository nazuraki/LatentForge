...
REPO_RAW="https://raw.githubusercontent.com/nazuraki/LatentForge/main"
...
add_profile() {
  ...
  case ",$existing," in
    *",$1,"*) ;;
    ",,") set_env COMPOSE_PROFILES "$1" ;;
    *) set_env COMPOSE_PROFILES "$existing,$1" ;;
  esac
}
...
# The caddy service bind-mounts this; fetch atomically so a failed download
# can't leave a truncated file behind for an already-running stack.
mkdir -p "$DIR/caddy"
tmp="$DIR/caddy/Caddyfile.tmp.$$"
curl -fsSL "$REPO_RAW/caddy/Caddyfile" -o "$tmp" \
  || { rm -f "$tmp"; fail "could not download caddy/Caddyfile from $REPO_RAW"; }
mv "$tmp" "$DIR/caddy/Caddyfile"
